...
// --- CHUNK_METADATA_END ---
= Доповнення (перед проміжним іспитом)
1. Повторення асимптотичної нормальності
2. Приклад
3. Асимптотичний півот
4. Приклад 2
...
// --- CHUNK_METADATA_END ---
== Асимптотичні властивості послідовності оцінювачів $(hat(theta)_n)_(n >= 1)$// --- CHUNK_METADATA_START ---
// needs_review: True
...
// --- CHUNK_METADATA_END ---
- Узгодженість $hat(theta)_n -->^P theta$// --- CHUNK_METADATA_START ---
// needs_review: True
...
// --- CHUNK_METADATA_END ---
- Асимптотична нормальність, якщо існує  $sigma^2 > 0$// --- CHUNK_METADATA_START ---
// needs_review: True
// src_checksum: 01ba4719c80b6fe911b091a7c05124b64eeece964e09c058ef8f9805daca546b
// --- CHUNK_METADATA_END ---
...
// --- CHUNK_METADATA_END ---
Загалом, с// --- CHUNK_METADATA_START ---
// needs_review: True
...
// --- CHUNK_METADATA_END ---
Кажуть, що // --- CHUNK_METADATA_START ---
// needs_review: True
...
// --- CHUNK_METADATA_END ---
 збігається зі швидкістю  // --- CHUNK_METADATA_START ---
// needs_review: True
...
// --- CHUNK_METADATA_END ---
#rmk[
  Якщо $hat(theta)_n$ асимптотично нормальний  $=>$  $hat(theta)_n$ консистентний 
  $
...
// --- CHUNK_METADATA_END ---
#underline[ $delta$-метод ]// --- CHUNK_METADATA_START ---
// needs_review: True
...
// --- CHUNK_METADATA_END ---
 диференційовна в  // --- CHUNK_METADATA_START ---
// needs_review: True
...
g диференційовна в // --- CHUNK_METADATA_START ---
// needs_review: True
...
#ex[
  $X_1, .., X_n$ з функцією щільності  $f(x) = 1/mu e^(-x/mu), x >= 0, mu = E[X_i] > 0$
   ...
   $mu$ оцінюється за допомогою  $hat(mu) = overline(X)$ ефективний?  $log L_n (mu) = - n log mu - 1/mu sum_(i=1)^n X_i $
    $
   ...
   ЦГТ: $sqrt(n)(hat(mu)_n - mu) -->^(cal(L)) cal(N) (0, mu^2) => Var(hat(mu)_n) = mu^2/n = $ дисперсія асимптотичного гаусового розподілу

    $( sqrt(n)(hat(mu)_n - mu) )/mu$ має асимптотичний розподіл $cal(N) (0, 1)$ 

  - інша параметризація: $(X_1, ..., X_n)$ н.о.р.  $f(x) = theta e^(-theta x), x >= 0$
   $
  ...
  #rmk[
    ...
    $n overline(X) ~ Gamma (n, theta)$
     $
    E[1/(n overline(X))] = theta/(n-1) "і" Var(1/((n overline(X))^2)) = theta^2/((n-1)(n-2))
    $ 
  ]
  $
...
// --- CHUNK_METADATA_END ---
== Півот (асимптотичний) або півотна статистика// --- CHUNK_METADATA_START ---
// needs_review: True
// src_checksum: 75a11da44c802486bc6f65640aa48a730f0f684c5c07a42ba3cd1735eb3fb070
// --- CHUNK_METADATA_END ---
...
// --- CHUNK_METADATA_END ---
#ex[
 $X_1, ..., X_n$ i.i.d.  $"Бернуллі"(theta)$ з  $theta in ]0, 1[$: 
  $
 ...
 $ 
 півотний метод для ДІ:
 Ми оцінюємо $sqrt(theta(1 - theta))$ за допомогою  $sqrt(hat(theta)(1 - hat(theta)))$ «plug-in» за допомогою LAC  $g(x) = sqrt(x(1 -x))$ з  $x in ]0, 1[$,  $sqrt(hat(theta)(1 - hat(theta)))$ узгоджена оцінка для  $sqrt(theta(1 - theta))$
  $
...
#ex[
  ...
  $ 
  $
  (log L_n)''(theta) = n/theta^2 - 2/theta^3 sum X_i^3; (log L_n)''(hat(theta)) = n/hat(theta)^2 - 2/hat(theta)^3 n hat(theta) = - n/hat(theta)^2 < 0 + "єдиність"
  $ 
  $=>$ глобальний максимум 
  ...
  $ 
  $q_(alpha/2)$ та  $q_(1 - alpha/2)$ квантилі  $cal(N) (0, 1)$
